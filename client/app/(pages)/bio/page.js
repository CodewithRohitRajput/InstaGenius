...
export default function BioGenerator() {
  ...
  const generateBios = async () => {
    ...
    try {
      const res = await fetch('https://instagenius.onrender.com/bio', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ 
          prompt: `Generate 5 Instagram bios for: ${name}. 
          Profession: ${profession}. 
          Interests: ${interests}.
          Tone: ${tone}.
          Include relevant emojis.
          Make them professional and platform-optimized.
          Format each bio with line breaks for readability
          Format: Each bio should be separated by exactly TWO newlines (no dashes or other separators)`
        })
      });

      const data = await res.json();
      const generatedBios = data.reply.split('\n\n')
        .filter(bio => bio.trim())
        .map(bio => ({
          text: bio.trim()
        }));
    ...
    <Navbar/>
      <div className="flex flex-col min-h-screen bg-white dark:bg-black mt-14">
        ...
        <p className="text-center text-black dark:text-white text-sm mt-2">
          AI-powered Instagram bio generator for creative, catchy, and optimized profiles.
        </p>
          ...
          <div className="max-w-2xl mx-auto">
            ...
            >
              <div className="p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    <label htmlFor="name" className="block text-sm font-medium text-black dark:text-white mb-2">
                      Your Name <span className="text-xs text-orange-500 dark:text-orange-400">(Required)</span>
                    </label>
                    <input
                      id="name"
                      type="text"
                  ...
                  <div>
                    <label htmlFor="profession" className="block text-sm font-medium text-black dark:text-white mb-2">
                      Profession
                    </label>
                    <input
                      id="profession"
                      type="text"
                      className="w-full text-black dark:text-white bg-white dark:bg-black border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all placeholder:text-gray-500 dark:placeholder:text-gray-400"
                      placeholder="E.g. Photographer, Entrepreneur"
                      value={profession}
                ...
                <div className="mb-6">
                  <label htmlFor="interests" className="block text-sm font-medium text-black dark:text-white mb-2">
                    Interests/Hobbies
                  </label>
                  <input
                    id="interests"
                ...
                <div className="mb-6">
                  <label className="block text-sm font-medium text-black dark:text-white mb-2">
                    Bio Tone <span className="text-xs text-orange-500 dark:text-orange-400">(Select one)</span>
                  </label>
                  ...
                  >
                    <div className="p-6">
                      <div className="flex items-center justify-between mb-4">
                        <h2 className="text-lg font-semibold text-black dark:text-white">Your Bios</h2>
                        <div className="flex space-x-3">
                          <button 
                            onClick={regenerateBios}
                        ...
                        <div className="space-y-4">
                          <AnimatePresence>
                            {bios.map(({text}, index) => (                            
                              ...
                              >
                                <div className="whitespace-pre-line text-black dark:text-white">{text}</div>
